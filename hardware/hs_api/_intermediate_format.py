    ...
    with open(conn_file_path, "r") as f:
        ...
        for line in lines:
            ...
            if "Axons" in line:  # parse for external inputs to the network
                ...
                "Neurons" in line
            ):  # parse for neuron-neuron synapses internal to the network
                parse_axons = False
                parse_neurons = True
...
def write_conn_file(B, W, opath="./test_connectivity.txt", print_to_screen=True):
    """
    ...
    if NW1 != N:
        print(
            "Error: different numbers of neurons defined in axon [{}x{}] and neuron [{}x{}] weight matrices!".format(
                M, N, NW1, NW2
            )
        )
    elif NW1 != NW2:
        print(
            "Error: neuron weight matrix must be square, but its [{}x{}].".format(
                NW1, NW2
            )
        )
    else:  # no errors
        ...
        for i in range(N):
            ...
            edges = (
                []
            )  # container for collecting all (pre-synaptic neuron, post-synaptic neuron, weight) tuples
            for j in range(N):
                weight = W[i][j]
                if weight != 0:  # only list non-zero weights
...
def write_input_file(Iext, opath="./test_inputs.txt", print_to_screen=True):
    """
    Write connectivity text file to disk, and optionally print the same to screen
