...
import ale_py
from conversion.sb3_to_sequential.sb3_to_sequential_converter import SequentialDQNNetwork, SB3ToSequentialConverter
...
def debug_sb3_vs_sequential():
    ...
    try:
        ...
        print("FAILED: Could not create environment, using dummy inputs")
        env = None
    ...
    try:
        ...
        print(f"SUCCESS: SNN model loaded: {type(snn_checkpoint)}")
        ...
        print(f"FAILED: Could not load SNN model: {e}")
        snn_model = None
    ...
    try:
        ...
        print("SUCCESS: Sequential model loaded successfully")
        print(f"Sequential model architecture:\n{seq_model}")
        
    except Exception as e:
        print(f"FAILED: Could not load Sequential model: {e}")
        seq_model = None
    ...
    try:
        if env is not None:
            ...
            print("SUCCESS: Fresh SB3 DQN created")
        ...
        print(f"FAILED: Could not create SB3 DQN: {e}")
        sb3_model = None
    ...
    if snn_model and seq_model:
        ...
        if diff < 1e-6:
            print("SUCCESS: Outputs match - architecture conversion is correct")
        else:
            print("FAILED: Outputs differ - there's an architecture mismatch!")
    ...
    if sb3_model and seq_model:
        ...
        # SB3 forward pass
        with torch.no_grad():
            sb3_q_values = sb3_model.q_net(test_input)
...
if __name__ == "__main__":
    ...
    print("\n=== SUMMARY ===")
    print(f"SNN model loaded: {'SUCCESS' if snn_model else 'FAILED'}")
    print(f"Sequential model loaded: {'SUCCESS' if seq_model else 'FAILED'}")
    print(f"SB3 model created: {'SUCCESS' if sb3_model else 'FAILED'}")
