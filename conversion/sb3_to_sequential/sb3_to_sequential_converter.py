...
class SequentialDQNNetwork(nn.Module):
    ...
    def __init__(self, input_channels: int = 4, n_actions: int = 6):
        ...
        self.network = nn.Sequential(OrderedDict([
            # Convolutional layers - match SB3 exactly
            ('conv1', nn.Conv2d(input_channels, 32, kernel_size=8, stride=4)),
            ('relu1', nn.ReLU()),  # NOT inplace to match SB3
            
            ('conv2', nn.Conv2d(32, 64, kernel_size=4, stride=2)),
            ('relu2', nn.ReLU()),  # NOT inplace to match SB3
            
            ('conv3', nn.Conv2d(64, 64, kernel_size=3, stride=1)),
            ('relu3', nn.ReLU()),  # NOT inplace to match SB3
            
            # Flatten layer
            ('flatten', nn.Flatten(start_dim=1, end_dim=-1)),  # Match SB3 exactly
            
            # Linear layers
            ('fc1', nn.Linear(3136, 512)),  # 64 * 7 * 7 = 3136
            ('relu4', nn.ReLU()),  # NOT inplace to match SB3
